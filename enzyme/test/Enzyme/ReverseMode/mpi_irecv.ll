...
; CHECK: define internal void @diffemsg1(i8** %ptr, i8** %"ptr'", i32 %numprocprec, i32 %etiquette, { { i8*, i64, i8*, i64, i64, i8*, i8 }*, i8*, i8* } %tapeArg)
; CHECK-NEXT: entry:
; CHECK-NEXT:   %0 = alloca %struct.ompi_status_public_t
; CHECK-NEXT:   %1 = alloca i32
; CHECK-NEXT:   %"malloccall'mi" = extractvalue { { i8*, i64, i8*, i64, i64, i8*, i8 }*, i8*, i8* } %tapeArg, 2
; CHECK-NEXT:   %"r2'ipc" = bitcast i8* %"malloccall'mi" to %struct.ompi_request_t**
; CHECK-NEXT:   %"into'il_phi" = extractvalue { { i8*, i64, i8*, i64, i64, i8*, i8 }*, i8*, i8* } %tapeArg, 1
; CHECK-NEXT:   %2 = extractvalue { { i8*, i64, i8*, i64, i64, i8*, i8 }*, i8*, i8* } %tapeArg, 0
; CHECK-NEXT:   %3 = icmp eq { i8*, i64, i8*, i64, i64, i8*, i8 }* %2, null
; CHECK-NEXT:   br i1 %3, label %invertentry_end, label %invertentry_nonnull

; CHECK: invertentry_nonnull:                              ; preds = %entry
; CHECK-NEXT:   %4 = load { i8*, i64, i8*, i64, i64, i8*, i8 }, { i8*, i64, i8*, i64, i64, i8*, i8 }* %2
; CHECK-NEXT:   %5 = bitcast { i8*, i64, i8*, i64, i64, i8*, i8 }* %2 to i8*
...
; CHECK-NEXT:   %[[i14:.+]] = call i32 @MPI_Wait(%struct.ompi_request_t** %"r2'ipc", %struct.ompi_status_public_t* %0)
; CHECK-NEXT:   %[[i15:.+]] = call i32 @MPI_Type_size(i8* bitcast (%struct.ompi_predefined_datatype_t* @ompi_mpi_real to i8*), i32* %1)
; CHECK-NEXT:   %[[i16:.+]] = load i32, i32* %1
; CHECK-NEXT:   %[[i17:.+]] = zext i32 %[[i16]] to i64
; CHECK-NEXT:   call void @llvm.memset.p0i8.i64(i8* nonnull %"into'il_phi", i8 0, i64 %[[i17]], i1 false)
